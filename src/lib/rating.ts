...
export const getRatingInfo = (rating: number) => {
  if (rating < 1200) return { title: 'Newbie', className: 'text-gray-400 font-semibold' }
  if (rating < 1400) return { title: 'Pupil', className: 'text-green-500 font-semibold' }
  if (rating < 1600) return { title: 'Specialist', className: 'text-cyan-400 font-semibold' }
  if (rating < 1900) return { title: 'Expert', className: 'text-blue-500 font-bold' }
  if (rating < 2100) return { title: 'Candidate Master', className: 'text-fuchsia-500 font-bold' }
  if (rating < 2300) return { title: 'Master', className: 'text-orange-400 font-bold' }
  if (rating < 2400) return { title: 'International Master', className: 'text-orange-500 font-extrabold' }
  if (rating < 2600) return { title: 'Grandmaster', className: 'text-red-500 font-bold' }
  if (rating < 3000) return { title: 'International Grandmaster', className: 'text-red-600 font-extrabold' }
  if (rating < 4000) return { title: 'Legendary Grandmaster', className: 'text-red-600 font-black drop-shadow-[0_0_8px_rgba(220,38,38,0.8)]' }
  return { title: 'Supreme', className: 'text-transparent bg-clip-text bg-gradient-to-r from-rose-500 via-amber-400 to-violet-500 font-black drop-shadow-[0_0_12px_rgba(245,158,11,0.6)]' }
}
